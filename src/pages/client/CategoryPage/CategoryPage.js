...
import { getCategoryById, getCategoryDocument } from "~/services/categoryService";
import { Col, Pagination, Row, Statistic, Button, Divider } from "antd";
import { EyeOutlined, DownloadOutlined, BookOutlined, ArrowDownOutlined } from "@ant-design/icons";
...
function CategoryPage() {
  ...
  const fetchData = async (page = 1, size = 8) => {
    ...
    try {
      ...
      const resDocs = await getCategoryDocument(categoryId, page - 1, size);
    ...
    <div className={cx("category-page")}>
      ...
      <div className={cx("hero-section")} ref={heroRef}>
        ...
        <div className={cx("hero-content", "animate__animated", "animate__fadeIn")}>
          ...
          <Button type="primary" size="large" className={cx("hero-button")} onClick={scrollToDocuments}>
      ...
      {loading ? (
        ...
        <>
          ...
          <div className={cx("cta-section", "wow", "animate__fadeIn")} data-wow-delay="0.2s">
            ...
            <Button type="primary" size="large" onClick={() => navigate("/")} className={cx("cta-button")}>
              Quay lại trang chủ
            </Button>
          </div>
        </>
